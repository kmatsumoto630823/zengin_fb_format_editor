...
const FBAttrs FBParser::fb_data_attrs_sohfuri =
{
    ...
    {10, "新規コード",     90,  1, char_num,   pad_num,   nullptr, "文字種（桁）：Ｎ（１）\r\n必須項目\r\n「１」：第１回振込分\r\n「２」：変更分（銀行・支店、預金種目・口座番号）\r\n「０」：その他"},
    {11, "顧客コード1",    91, 10, char_kana,  pad_kana,  nullptr, "文字種（桁）：Ｎ（１０）\r\n任意項目\r\n依頼人が定めた受取人識別のための顧客コード"},
    {12, "顧客コード2",   101, 10, char_kana,  pad_kana,  nullptr, "文字種（桁）：Ｎ（１０）\r\n任意項目\r\n依頼人が定めた受取人識別のための顧客コード"},
...
const FBAttrs FBParser::fb_header_attrs_kyuyo_shoyo =
{
    ...
    { 1, "種別コード",          1,  2, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（２）\r\n必須項目\r\n「１１」：給与振込（民間）\r\n「１２」：給与振込（民間）\r\n「７１」：給与振込（地方公務員）\r\n「７２」：賞与振込（地方公務員）"},
    { 2, "コード区分",          3,  1, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（１）\r\n必須項目\r\n「０」：ＳＪＩＳ\r\n「１」：ＥＢＣＤＩＣ"},
    { 3, "会社コード",          4, 10, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（１０）\r\n必須項目\r\n銀行が採番した取引先の会社コード"},
    { 4, "会社名",             14, 40, char_kana,  pad_kana,  nullptr,   "文字種（桁）：Ｃ（４０）\r\n必須項目"},
    { 5, "振込指定日",         54,  4, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（４）\r\n必須項目\r\nＭＭＤＤ（ＭＭ月ＤＤ日）"},
    { 6, "仕向銀行番号",       58,  4, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（４）\r\n必須項目\r\n統一金融機関番号"},
    { 7, "仕向銀行名",         62, 15, char_kana,  pad_kana,  nullptr,   "文字種（桁）：Ｃ（１５）\r\n任意項目"},
    { 8, "仕向支店番号",       77,  3, char_num,   pad_num,   nullptr,   "文字種（桁）：Ｎ（３）\r\n必須項目\r\n統一店番号"},
    { 9, "仕向支店名",         80, 15, char_kana,  pad_kana,  nullptr,   "文字種（桁）：Ｃ（１５）\r\n任意項目"},
    {10, "預金種目（企業等）", 95,  1, char_num,   pad_num,   " ",       "文字種（桁）：Ｎ（１）\r\n任意項目\r\n「１」：普通預金\r\n「２」：当座預金"},
    {11, "口座番号（企業等）", 96,  7, char_num,   pad_num,   "       ", "文字種（桁）：Ｎ（７）\r\n任意項目"},
    {12, "ダミー",            103, 17, char_dummy, pad_dummy, nullptr,   "文字種（桁）：Ｃ（１７）\r\n必須項目\r\nスペースとする"}
};
...
const FBAttrs FBParser::fb_data_attrs_kyuyo_shoyo =
{
    ...
    { 6, "預金種目",       42,  1, char_num,   pad_num,   nullptr,      "文字種（桁）：Ｎ（１）\r\n必須項目\r\n「１」：普通預金\r\n「２」：当座預金"},
    { 7, "口座番号",       43,  7, char_num,   pad_num,   nullptr,      "文字種（桁）：Ｎ（７）\r\n必須項目"},
    { 8, "預金者名",       50, 30, char_kana,  pad_kana,  nullptr,      "文字種（桁）：Ｃ（３０）\r\n必須項目"},
    { 9, "振込金額",       80, 10, char_num,   pad_num,   nullptr,      "文字種（桁）：Ｎ（１０）\r\n必須項目"},
    {10, "新規コード",     90,  1, char_num,   pad_num,   nullptr,      "文字種（桁）：Ｎ（１）\r\n必須項目\r\n「１」：第１回振込分\r\n「２」：変更分（銀行・支店、預金種目・口座番号）\r\n「０」：その他"},
    {11, "社員番号",       91, 10, char_num,   pad_num,   "          ", "文字種（桁）：Ｎ（１０）\r\n任意項目\r\n企業等での社員番号"},
    {12, "所属コード",    101, 10, char_num,   pad_num,   "          ", "文字種（桁）：Ｎ（１０）\r\n任意項目\r\n企業等での所属コード"},
    {13, "ダミー",        111,  9, char_dummy, pad_dummy, nullptr,      "文字種（桁）：Ｃ（９）\r\n必須項目\r\nスペースとする"}
};
